...
**Pass 1 — Safety-first STPA (from docs/safety/functional-safety-approach.md):**
...
**Pass 2 — Security-informed STRIDE (from docs/security/ot-security-architecture.md):**
...
TLC [G1]: [CPS product/system] is acceptably safe AND secure for operation in [context].
  ...
  Strategy [S1]: Argue by safety-security interaction — all identified interactions are resolved
    ...
    Sub-claim [G2]: All safety hazards are identified and mitigated (safety case)
      [Decompose per docs/safety/functional-safety-approach.md hazard register]
...
- **docs/safety/functional-safety-approach.md** (`docs/` in this repo): Source of hazard register, safety functions, SIL allocation, UCAs.
- **docs/security/ot-security-architecture.md** (`docs/` in this repo): Source of zone/conduit model, threat model, SL-T allocation.
