...
- **docs/safety/functional-safety-approach.md** (`docs/` in this repo): Safety functions must reside in protected zones. SL-T for safety zones must ensure security cannot defeat safety integrity.
- **docs/compliance/conformity-assessment-guide.md** (`docs/` in this repo): CRA applies to OT products placed on EU market. NIS2 applies to operators of OT in essential/important entities.
